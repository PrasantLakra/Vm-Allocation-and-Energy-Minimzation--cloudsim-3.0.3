...
public class Helper {
	...
	public static void printResults(
	        ...
	        boolean outputInCsv,
	        String outputFolder,
	        double referenceEnergyConsumption)  {
		
		 double energyConsumption = datacenter.getPower() / (3600 * 1000); // Energy consumption in kWh
		 double energyEfficiency1 = referenceEnergyConsumption / energyConsumption; // Energy efficiency
		//  double energyConsumption = datacenter.getPower() * lastClock / (3600 * 1000); // Energy consumption in kWh 
		if (energyConsumption == 0) {
	        energyConsumption = 1; // Set a default value to avoid division by zero
	    }
	...
	//		data.append(String.format("%.5f", energyPercentage) + delimeter);
			...
			Log.printLine();
			Log.printLine(String.format("Experiment Name:- Inter-Quartile Range(IQR_Test) "));
			Log.printLine(String.format("Number of hosts: " + numberOfHosts));
			Log.printLine(String.format("Number of VMs: " + numberOfVms));
			Log.printLine(String.format("Energy consumption: %.2f kWh", energyConsumption));
	        Log.printLine(String.format("Energy Efficiency: %.2f%%", energyEfficiency1));
